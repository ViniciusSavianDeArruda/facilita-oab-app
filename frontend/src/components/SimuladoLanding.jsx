...
const MATERIAS = [
  ...
  'Penal', 'Processo Penal', 'Administrativo', 'Tributário',
...
export default function SimuladoLanding({ onStart }) {
  ...
  async function iniciar() {
    ...
    try {
      ...
      setError(e.message || 'Não deu pra gerar o simulado.')
      setLoading(false)
    }
  }
  ...
  if (loading) {
    return (
      <div className="max-w-xl mx-auto pt-24 px-6 text-center">
        <div className="font-serif text-2xl text-cream-50 mb-3" style={{ fontVariationSettings: '"opsz" 60' }}>
          Preparando as questões
        </div>
        <p className="text-cream-400 text-sm mb-8">
          O mentor está redigindo dez questões inéditas. Isso costuma levar 10-20 segundos.
        </p>
        <div className="flex justify-center gap-2">
          <span className="w-2 h-2 rounded-full bg-brass animate-pulse" style={{ animationDelay: '0ms' }}></span>
          <span className="w-2 h-2 rounded-full bg-brass animate-pulse" style={{ animationDelay: '200ms' }}></span>
          <span className="w-2 h-2 rounded-full bg-brass animate-pulse" style={{ animationDelay: '400ms' }}></span>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-xl mx-auto pt-16 px-6">
      <h2 className="font-serif text-3xl md:text-4xl text-cream-50 leading-tight tracking-tight mb-3" style={{ fontVariationSettings: '"opsz" 96' }}>
        Dez questões, quinze minutos.
      </h2>
      <p className="text-cream-400 leading-relaxed mb-10">
        Cada simulado tem dez questões inéditas geradas no estilo FGV. Ao final, você vê o gabarito, a explicação e pode conversar com o mentor sobre qualquer erro.
      </p>
...
function ModoCard({ selected, onClick, title, subtitle }) {
  return (
    ...
    >
      <div className={`font-serif text-lg ${selected ? 'text-brass' : 'text-cream-50'}`} style={{ fontVariationSettings: '"opsz" 60' }}>
        {title}
      </div>
